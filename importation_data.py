...
def importer_donnees(connexion):
    if connexion:
        ...
        fichiers_tsv = ['title.basics.tsv.gz', 'title.principals.tsv.gz', 'title.akas.tsv.gz', 'title.akas.tsv.gz', 'title.crew.tsv.gz', 'title.episode.tsv.gz', 'title.ratings.tsv.gz', 'name.basics.tsv.gz'] 
        for fichier in fichiers_tsv:
            ...
            requete_tables = f"TRUNCATE TABLE netflox_test_p.{nom_table};"
            ...
            try:
                df.to_sql(nom_table, engine, schema='netflox_test_p', if_exists='append', index=False)
...
def observer_nombre_lignes_et_taux_remplissage(connexion):
    try:
        curseur = connexion.cursor()
        requete_tables = "SELECT table_name FROM information_schema.tables WHERE table_schema = 'netflox_test_p';"
        curseur.execute(requete_tables)
        ...
        for table in tables:
            nom_table = table[0]
            requete_count = f"SELECT COUNT(*) FROM netflox_test_p.{nom_table};"
            curseur.execute(requete_count)
            nombre_lignes = curseur.fetchone()[0]
            print(f"Table {nom_table} a {nombre_lignes} lignes.")
            
            requete_taille = f"SELECT pg_total_relation_size('netflox_test_p.{nom_table}');"
            curseur.execute(requete_taille)
            taille_table = curseur.fetchone()[0]
            total_taille += taille_table 
            
            requete_taille_remplie = f"SELECT pg_relation_size('netflox_test_p.{nom_table}');"
