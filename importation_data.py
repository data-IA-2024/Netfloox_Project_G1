...
def importer_donnees(connexion):
    if connexion:
        ...
        fichiers_tsv = ['title.basics.tsv.gz', 'title.principals.tsv.gz', 'title.akas.tsv.gz'] 
        for fichier in fichiers_tsv:
            ...
            requete_tables = f"TRUNCATE TABLE netfloox.{nom_table};"
            curseur.execute(requete_tables)
            connexion.commit()
            df = pd.read_csv(chemin_fichier, sep='\t', compression='gzip', low_memory=False, nrows=n_rows, na_values=['\\N'], on_bad_lines='skip' )
            

            try:
                df.to_sql(nom_table, engine, schema='netfloox', if_exists='append', index=False)
                print(f"Table {nom_table} remplie avec succès.")
            except Exception as e:
                print(f"Erreur lors du remplissage de la table {nom_table}: {e}")
        

def observer_nombre_lignes_et_taux_remplissage(connexion):
    try:
        curseur = connexion.cursor()
        requete_tables = "SELECT table_name FROM information_schema.tables WHERE table_schema = 'netfloox';"
        curseur.execute(requete_tables)
...
def creation_cle_etrangere(connexion):
    pass
        

def analyser_donnees(connexion):
    try:
        curseur = connexion.cursor()
        
        # Nettoyage des données
        requete_nettoyage = """
        DELETE FROM netfloox.title_basics WHERE "primaryTitle" IS NULL OR "primaryTitle" = '';
        DELETE FROM netfloox.title_ratings WHERE "averageRating" IS NULL OR "numVotes" IS NULL;
        """
        curseur.execute(requete_nettoyage)
        connexion.commit()
        ...
        FROM netfloox.title_basics
        GROUP BY "primaryTitle"
        ORDER BY count DESC
        LIMIT 100000;
        """
        curseur.execute(requete_analyse)
        resultats = curseur.fetchall()
        
        for resultat in resultats:
            print(f"Titre: {resultat[0]}, Nombre: {resultat[1]}")
        
        curseur.close()
        connexion.close()
    except psycopg2.Error as e:
        print(f"Erreur lors du nettoyage et de l'analyse des données: {e}")
        connexion.close()

def nettoyer_donnees(connexion):
    pass    

def sauvegarder_donnees(connexion):
    pass    
